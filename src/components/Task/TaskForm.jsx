...
export default function TaskForm({ user, navigate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validSteps.length === 0) return alert('ステップを少なくとも1つ入力してください。');
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                      ...
                      <button type="button" className="btn btn-sm" style={{ background: 'none', padding: '2px 6px', fontSize: '16px', color: 'var(--danger)' }} onClick={() => removeStep(step.id)} disabled={steps.length === 1}>×</button>
                    </div>
                  </div>
                  <input
                    type="text"
